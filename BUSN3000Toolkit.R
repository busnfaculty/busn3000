...
##   bar.chart()   bar chart for a categorical variable (auto headroom + wrap)
...
## =====================================================================
## bar.chart()  —  bar chart for one categorical variable
##
##   x         a vector / data-frame column (e.g. d1$q05_class_standing)
##   main      optional title
...
##   wrap      wrap category labels to this many characters (default 12)
##   cex       text size for labels / axis / title (default 0.85)
##
## Automatically leaves headroom above the tallest bar and wraps long
## category labels onto multiple lines so none are dropped. Pairs with
## png.print():
##   bar.chart(d1$q05_class_standing, main = "Class Standing")
##   png.print("standing_bar.png")
## =====================================================================

bar.chart <- function(x, main = NULL, xlab = NULL, ylab = "Frequency",
                      col = "steelblue", wrap = 12, cex = 0.85) {

  if (is.null(xlab)) xlab <- sub(".*\\$", "", deparse(substitute(x)))

  counts <- table(x)
  labs   <- vapply(names(counts),
  ...
  mp <- barplot(counts, axisnames = FALSE,
                main = main, xlab = "", ylab = ylab, col = col,
                ylim = c(0, ymax * 1.15),
                cex.axis = cex, cex.lab = cex, cex.main = cex * 1.3)

  ## draw wrapped labels under each bar with text() (never dropped)
  text(mp, par("usr")[3] - 0.02 * ymax, labels = labs,
       xpd = NA, adj = c(0.5, 1), cex = cex)
  mtext(xlab, side = 1, line = 4, cex = cex)     # x-axis title below the labels
