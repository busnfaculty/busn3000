...
bar.chart <- function(x, group = NULL, main = NULL, xlab = NULL,
  ...
  ## x positions of the category labels (grouped: center of each bar group)
  at <- if (grouped) colMeans(mp) else as.vector(mp)
  text(at, par("usr")[3] - 0.02 * ymax, labels = labs,
       xpd = NA, adj = c(0.5, 1), cex = cex.axis)
  mtext(xlab, side = 1, line = 4, cex = cex.axis)
