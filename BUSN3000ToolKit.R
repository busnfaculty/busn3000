...
.btext <- function(cx, j, align, y, label, cex) {
  st <- .btable_style
  if (align == "left")
    text(cx[j] + st$cell_pad_x,     y, label, adj = c(0, 0.5),   cex = cex)
  else if (align == "right")
    text(cx[j + 1] - st$cell_pad_x, y, label, adj = c(1, 0.5),   cex = cex)
  else
...
## draw the table, centered, on the current device
.draw_btable <- function(spec) {
  ...
  x_left <- (din[1] - L$total_w) / 2
  cx     <- x_left + c(0, cumsum(L$colw))
  x0 <- cx[1]; x1 <- cx[length(cx)]
  y  <- (din[2] + L$total_h) / 2

  ## title
  if (!is.null(spec$title)) {
    text((x0 + x1) / 2, y - L$title_h / 2, spec$title, cex = st$title_cex, adj = c(0.5, 0.5))
    y <- y - L$title_h
  }
  ...
  ## which columns sit under a spanner
  span_cols <- integer(0)
  ...
  if (L$has_span) {
    ...
    for (sp in spec$spanners) {
      idx <- sp$cols
      text((cx[min(idx)] + cx[max(idx) + 1]) / 2, y - L$span_h / 2, sp$label,
           cex = st$head_cex, adj = c(0.5, 0.5))
      segments(cx[min(idx)] + st$cell_pad_x, yb, cx[max(idx) + 1] - st$cell_pad_x, yb, lwd = st$rule_mid)
    }
    y <- yb
  }
  ...
  full_bot <- y - L$head_h
  for (j in seq_len(ncol)) {
    yc <- if (j %in% span_cols) y - L$head_h / 2 else (full_top + full_bot) / 2
    .btext(cx, j, spec$align[j], yc, spec$header[j], st$head_cex)
  }
  y <- y - L$head_h
  segments(x0, y, x1, y, lwd = st$rule_thick)
  body_top <- y
  ...
  for (i in seq_along(spec$body)) {
    ...
    ym <- y - rh / 2
    if (identical(r$type, "group")) {
      text(cx[1] + st$cell_pad_x, ym, r$label, adj = c(0, 0.5), cex = st$body_cex, font = 3)
    } else {
      for (j in seq_len(ncol)) {
        lines <- strsplit(r$cells[j], "\n", fixed = TRUE)[[1]]
        if (!length(lines)) lines <- ""
        n <- length(lines)
        for (k in seq_len(n)) {
          yk <- ym + ((n - 1) / 2 - (k - 1)) * L$lh
          .btext(cx, j, spec$align[j], yk, lines[k], st$body_cex)
        }
      }
    }
  ...
  ## bottom rule + first-column divider
  segments(x0, y, x1, y, lwd = st$rule_thick)
  ...
  if (!is.null(spec$footnote))
    text(cx[1], y - L$foot_h / 2, spec$footnote, adj = c(0, 0.5), cex = st$foot_cex, col = "gray30")
...
c.table <- function(row, col, strata = NULL, main = NULL,
  ...
  panel <- function(r, c) {
    ...
    for (i in seq_len(R)) {
      ...
      for (j in seq_len(C))
        cells[j + 1] <- fmt_cell(tab[i, j], tp = tab[i, j] / grand * 100,
                                 rp = tab[i, j] / rowtot[i] * 100, cp = tab[i, j] / coltot[j] * 100)
      cells[C + 2] <- fmt_cell(rowtot[i], tp = rowtot[i] / grand * 100)
      rows[[i]] <- cells
    }
    tr <- character(C + 2); tr[1] <- "Total"
    for (j in seq_len(C)) tr[j + 1] <- fmt_cell(coltot[j], tp = coltot[j] / grand * 100)
    tr[C + 2] <- fmt_cell(grand, tp = 100)
    rows[[R + 1]] <- tr
